---
type: Decision
title: Nytka does not mandate a task tracker
status: stable
...
# 0004 — Nytka does not mandate a task tracker

## Decision

`project.yaml → tasks.tracker` declares the tracker (`file`, `github-projects`, `linear`,
`jira`). Nytka specifies the *contract* — minimum fields, and that status has exactly one
...
- When an external tracker is used, the repo may hold only a *generated read-only* snapshot,
  ...
  prevents.
- Nytka ships no tracker integration. A snapshot generator is per-project.
